...
function makeFront(trie,depth){
    ...
    let query = queries[depth];
    let c = query.charAt(depth);
    
    if(trie.has(c)){
        let tempTrie = trie.get(c)
        makeFront(tempTrie,depth+1);
    }
    else{
...
function makeTrie(){
    for(let i =0;i<queries.length;i++){
        let query = queries[i];
        makeFront(frontTrie,0);
    }

}

function findWord(trie,depth){
...
function solution(w, q) {
    ...
    for(let i =0;i<queries.length;i++){
        ...
        tempNum = getAnswer2(frontTrie,query);
        answer.push(tempNum);
    }
